import type { EmailTemplateParams, emailTemplatesType } from '../types/workflow.types';
            ...
            <tr>
                <td style="background-color: #f0f7ff; padding: 20px; text-align: center; font-size: 14px;">
                    <p style="margin: 0 0 10px;">
                        SubTrack Inc. | No 10 Shifau Cresent
                    </p>
                    <p style="margin: 0;">
...
export const emailTemplates: emailTemplatesType = [
    {
        ...
        generateBody: (data: EmailTemplateParams) => generateEmailTemplate({...data, daysLeft: 7})
    },
    
    {
        label: "5 days before reminder",
        generateSubject: (data: EmailTemplateParams) => 
            `Reminder: Your ${data.subscriptionName} Renews in 5 days - Stay Subscribed!`,
        generateBody: (data: EmailTemplateParams) => generateEmailTemplate({...data, daysLeft: 5})
    },
    {
        label: "2 days before reminder",
        generateSubject: (data: EmailTemplateParams) => 
            `2 Days Left! ${data.subscriptionName} Subscription Renewal`,
        generateBody: (data: EmailTemplateParams) => generateEmailTemplate({...data, daysLeft: 2})
    },
    {
        label: "1 day before reminder",
        generateSubject: (data: EmailTemplateParams) => 
            `Final Reminder: ${data.subscriptionName} Renews Tomorrow!`,
        generateBody: (data: EmailTemplateParams) => generateEmailTemplate({...data, daysLeft: 1})
    }
];